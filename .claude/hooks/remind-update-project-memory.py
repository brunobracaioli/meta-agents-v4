...
It listens to the deterministic success signals of the two skills:
  - create-traffic-brunobracaioli-campaign  -> `ads_create_campaign` MCP call,
    or a `*-trafego.json` run manifest is written.
  - analytic-traffic-brunobracaioli-campaign -> `execute_sql` that inserts into
...
REMINDERS: dict[str, str] = {
    "creation": (
        ...
        "tráfego Meta Ads (brunobracaioli/CCA). Conforme o CLAUDE.md "
    ...
    "analysis": (
        ...
        "performance (tabela analyses, brunobracaioli/CCA). Conforme o "

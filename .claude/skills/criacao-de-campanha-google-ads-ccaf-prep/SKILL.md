...
4. **Persista em `operation_logs` via REST** — no headless o MCP do Supabase é OAuth-gated
   e NÃO está disponível; use `curl` com os envs que o runner já tem (`SUPABASE_URL` +
   `SUPABASE_SECRET_KEY`). **Uma linha por entidade criada** (ou uma única de skip):

   ```bash
   curl -fsS -X POST "${SUPABASE_URL%/}/rest/v1/operation_logs" \
     -H "apikey: ${SUPABASE_SECRET_KEY}" \
     -H "Authorization: Bearer ${SUPABASE_SECRET_KEY}" \
     -H "Content-Type: application/json" \
     -d '{"client_id":"fe1b93e9-2f23-4949-897e-59a5c5130788","entity_type":"campaign",
          "meta_entity_id":"customers/4342319594/campaigns/<id>","action":"create",
